...
Distinguish two very different kinds of material before deciding what counts as a genuine blocking uncertainty: (1) something the Landscape itself has already established as a real, unresolved fact about the decision as framed (a strong candidate for blocking), versus (2) something one panel lens (Guardian, Pragmatist, or Empathiser) merely raised as a soft, speculative consideration or side-factor, without it being established that this is actually part of the decision as framed. Do NOT promote (2) into a blocking uncertainty just because a lens mentioned it - that overreaches beyond assessing the reasoning given into inventing scope the decision itself was never framed to include. Check the governing objective directly: if it doesn't establish that a particular person, fact, or consideration is actually part of this decision, a passing mention of it elsewhere is not, on its own, a blocking uncertainty - at most it may belong in missingInformation as a minor, non-blocking note, if it belongs anywhere at all.

When a decision is framed in the first person singular about the person's own choice, their own genuine desire or intention is normally the most foundational uncertainty - other people's views (a partner's, a family member's) are usually real but secondary considerations, relevant only once the person's own stance is established. Order blockingUncertainties accordingly - do not place another person's view ahead of the decision-maker's own stated or unstated position on the matter, unless the prompt itself frames the decision as inherently joint (e.g. "should we...").

Before finalising blockingUncertainties and missingInformation, cross-check each candidate against Landscape's own resolvedUncertainties. If Landscape has already resolved something (including via a recent clarifying answer), do not list it again as blocking or missing, even if one of Guardian/Pragmatist/Empathiser's individual outputs still mentions or assumes it - those lenses may not have fully updated their own framing yet, but Landscape's resolvedUncertainties is the authoritative, current record of what is now settled. If you notice a lens's output still resting on something Landscape has since resolved (e.g. still assuming a partner exists after that was found to be unconfirmed), that tension is worth noting as a reason for INCONSISTENT internal consistency - but it does not mean the resolved fact should be re-listed as still blocking.
